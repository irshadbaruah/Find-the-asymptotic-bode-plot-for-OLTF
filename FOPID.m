...
% combined = mag1 + mag2 + mag3 + mag4 + mag5 + mag6 ;
 combined = mag1 + mag3 + mag4 + mag5 + mag6 ;
...
w=logspace(-2,2,1000);
Mag=@(w) (20*log10(abs(sqrt(Kp).^2)) + 20*log10(abs(sqrt(K).^2)) + 20*log10(abs(sqrt((Kd/Kp)*(i.*w).^(mew+ita) + ((i.*w).^mew) + (Ki/Kp)).^2)) - 20*log10(abs(sqrt((T*(i.*w)) + 1).^2)) - 20*log10(abs(sqrt(i*w).^mew).^2));   
